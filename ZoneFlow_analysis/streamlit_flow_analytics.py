...
class ZoneFlowTracker:
    ...
    def analyze_crossing(self, track_id, path):
        ...
        for zone_id, zone in enumerate(self.zones):
            was_inside = self.is_inside_polygon(prev_center, zone)
            is_inside = self.is_inside_polygon(curr_center, zone)
    ...
    def update_object_path(self, track_id, center):
        ...
        self.analyze_crossing(track_id, self.object_paths[track_id])
...
def draw_zones(frame, zones):
    ...
    for i, zone in enumerate(zones):
        ...
        # Add zone label
        if len(zone) > 0:
            label_pos = (int(zone[0][0]), int(zone[0][1]) - 10)
...
def perform_inference(frame, model, names, tracker, zone_tracker, conf_threshold=0.5, iou_threshold=0.45):
    ...
    for track in tracks:
        ...
        # Update zone tracker
        zone_tracker.update_object_path(track_id, center)
...
def display_zone_analytics(zone_tracker):
    """Display zone analytics in sidebar"""
    if not zone_tracker.zones:
        st.sidebar.info("No zones defined yet")
        return
    
    st.sidebar.header("📊 Zone Analytics")
    ...
    col2.metric("Total Exits", total_exits)
    col3.metric("Current Objects", total_current)
    
    # Per-zone analytics
    for zone_id, analytics in zone_tracker.zone_analytics.items():
        if zone_id < len(zone_tracker.zones):
            with st.sidebar.expander(f"Zone {zone_id + 1}"):
                st.metric("Entered", analytics['entered'])
                st.metric("Exited", analytics['exited'])
                st.metric("Currently Inside", analytics['current'])
                
                # Recent activity
                if analytics['history']:
                    st.write("Recent Activity:")
                    recent = analytics['history'][-5:]  # Last 5 events
                    for event in reversed(recent):
                        timestamp = event['timestamp'].strftime("%H:%M:%S")
                        action_color = "🟢" if event['action'] == 'entered' else "🔴"
                        st.write(f"{action_color} {event['action']} at {timestamp}")
...
def create_analytics_dashboard(zone_tracker):
    ...
    for zone_id, analytics in zone_tracker.zone_analytics.items():
        if zone_id < len(zone_tracker.zones):
            zone_data.append({
                'Zone': f'Zone {zone_id + 1}',
                'Entered': analytics['entered'],
                'Exited': analytics['exited'],
    ...
    if zone_data:
        ...
        with col1:
            ...
            fig_bar = px.bar(df, x='Zone', y=['Entered', 'Exited'], 
                           title='Zone Entry/Exit Analysis',
                           barmode='group')
            st.plotly_chart(fig_bar, use_container_width=True)
        
        with col2:
            # Current occupancy
            fig_current = px.bar(df, x='Zone', y='Current',
        ...
        timeline_data = []
        for zone_id, analytics in zone_tracker.zone_analytics.items():
            for event in analytics['history'][-50:]:  # Last 50 events
        ...
        if timeline_data:
            ...
            fig_timeline = px.scatter(timeline_df, x='Time', y='Zone', 
                                    color='Action', size_max=10,
                    ...
                    if "path" in obj:
                        ...
                        if len(points) >= 3:
                            ...
                            if "left" in obj and "top" in obj:
                                transformed_points = []
                                for x, y in points:
                                    new_x = x + obj.get("left", 0)
                                    new_y = y + obj.get("top", 0)
                                    transformed_points.append((int(new_x), int(new_y)))
                                zones.append(transformed_points)
                            else:
                                zones.append([(int(x), int(y)) for x, y in points])
...
def main():
    ...
    with st.sidebar:
        ...
        device = st.selectbox("Device", ["cpu", "0", "1"], index=0)
        ...
        if st.session_state.zone_definition_mode:
            ...
            drawing_mode = st.radio(
                "Drawing Method:",
                ["Interactive Canvas (Polygons)", "Interactive Canvas (Rectangles)", "Manual Coordinates"],
                index=0,
    ...
    # Load model
    with st.spinner(f"Loading YOLO model: {model_name}..."):
        model, names = load_yolo_model(model_name, device)
        if not model:
    ...
    with col1:
        if st.session_state.zone_definition_mode:
            ...
            if 'current_frame' in st.session_state:
                ...
                if drawing_mode.startswith("Interactive Canvas"):
                    ...
                    draw_zones(frame_copy, st.session_state.zone_tracker.zones)
            ...
            if start_button or st.session_state.get('analysis_running', False):
                ...
                while st.session_state.analysis_running and not stop_button:
                    ...
                    draw_zones(frame, st.session_state.zone_tracker.zones)
                    ...
                    # Update analytics in the side panel
                    if frame_count % 15 == 0:  # Update every 15 frames
                        with col2:
    ...
    with col2:
        ...
        if not st.session_state.zone_definition_mode:
            create_analytics_dashboard(st.session_state.zone_tracker)
        else:
